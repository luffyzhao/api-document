<template>
<div>
  <Modal title="修改权限" v-model="modalShow" :mask-closable="false" @on-visible-change="visibleChange" class-name="vertical-center-modal" width="auto" @on-ok="submit" ok-text="提交">
...
export default {
  ...
  methods: {
    ...
    submit(name){
      ...
      this.$refs[name].validate((valid) => {
          if (valid) {
            this.$put(`admin/permission/${this.updateId}`, {
  ...
  watch: {
    ...
    modalShow: function (val, oldVal) {
      if(val){
        this.$get(`admin/permission/${this.updateId}`).then((res) => {
