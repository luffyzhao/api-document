    ...
    submit(name){
      this.$refs[name].validate((valid) => {
          if (valid) {
            this.loading = true;
            if(this.data.id > 0){
              this.$put(`admin/book/${this.$route.query.id}/document/${this.data.id}`, this.addApiModel).then((res) => {
              ...
              this.$post(`admin/book/${this.$route.query.id}/document`, this.addApiModel).then((res) => {
                this.documents = res.data;
                this.loading = false;
                this.$Message.success('请求成功!');
                this.visibleChange(false)
