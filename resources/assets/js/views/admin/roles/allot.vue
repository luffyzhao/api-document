<template>
<div>
  <Modal title="分配权限" v-model="modalShow" :mask-closable="false" @on-visible-change="visibleChange" class-name="vertical-center-modal" width="500" @on-ok="submit" ok-text="提交">
      <Form ref="formItem" :model="formItem" :label-width="1">
        <FormItem>
          <CheckboxGroup v-model="formItem.data">
            <Checkbox v-for="item in formItem.permission" :key="item.id" :label="item.id">{{ item.display_name }}</Checkbox>
          </CheckboxGroup>
        </FormItem>
      </Form>
      <div slot="footer">
...
export default {
  ...
  watch: {
    ...
    modalShow: function (val, oldVal) {
      if(val){
        this.$get(`admin/permission/all`).then((res) => {
          this.formItem.permission = res.data;
        })
        this.$get(`admin/role/${this.allotId}/permission`).then((res) => {
