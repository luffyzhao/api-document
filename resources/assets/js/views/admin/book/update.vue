<template>
<div>
  <Modal title="修改项目" v-model="modalShow" :mask-closable="false" @on-visible-change="visibleChange" class-name="vertical-center-modal" width="auto" @on-ok="submit" ok-text="提交">
...
<script>
import markdown from '@/views/admin/components/markdown.vue'
export default {
  props: {
    show: {
      type: Boolean,
  ...
  methods: {
    ...
    submit(name){
      ...
      this.$refs[name].validate((valid) => {
          if (valid) {
            this.$put('admin/book/' + this.updateId, {
  ...
  watch: {
    modalShow: function (val, oldVal) {
      if(val){
          this.$get('admin/book/' + this.updateId).then((res) => {
