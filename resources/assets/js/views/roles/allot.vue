<template>
<div>
  <Modal title="修改文档" v-model="modalShow" :mask-closable="false" @on-visible-change="visibleChange" class-name="vertical-center-modal" width="500" @on-ok="submit" ok-text="提交">
...
export default {
  ...
  watch: {
    ...
    modalShow: function (val, oldVal) {
      if(val){
        this.$get(`permission/all`).then((res) => {
          this.formItem.permission = res.data;
        })
        this.$get(`role/${this.allotId}/allot`).then((res) => {
